...
/**
 * This is the model class for table "et_ophnuintraoperative_identifiers".
 *
...
class Element_OphNuIntraoperative_Identifiers  extends  BaseEventTypeElement
{
	...
	{
		return 'et_ophnuintraoperative_identifiers';
	}
	...
	public function getophnuintraoperative_identifiers_idoptions_defaults() {
		$ids = array();
	...
	protected function afterSave()
	{
		if (!empty($_POST['MultiSelect_idoptions'])) {
			...
			foreach (Element_OphNuIntraoperative_Identifiers_Idoptions_Assignment::model()->findAll('element_id = :elementId', array(':elementId' => $this->id)) as $item) {
				$existing_ids[] = $item->ophnuintraoperative_identifiers_idoptions_id;
			}
			...
			foreach ($_POST['MultiSelect_idoptions'] as $id) {
				if (!in_array($id,$existing_ids)) {
					...
					$item->ophnuintraoperative_identifiers_idoptions_id = $id;
			...
			foreach ($existing_ids as $id) {
				if (!in_array($id,$_POST['MultiSelect_idoptions'])) {
					$item = Element_OphNuIntraoperative_Identifiers_Idoptions_Assignment::model()->find('element_id = :elementId and ophnuintraoperative_identifiers_idoptions_id = :lookupfieldId',array(':elementId' => $this->id, ':lookupfieldId' => $id));
					if (!$item->delete()) {
