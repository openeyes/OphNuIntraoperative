...
/**
 * This is the model class for table "et_ophnuintraoperative_aditionals".
 *
...
class Element_OphNuIntraoperative_Aditionals  extends  BaseEventTypeElement
{
	...
	{
		return 'et_ophnuintraoperative_aditionals';
	}
	...
	public function getophnuintraoperative_aditionals_aditionals_defaults() {
		$ids = array();
	...
	protected function afterSave()
	{
		if (!empty($_POST['MultiSelect_aditionals'])) {
			...
			foreach (Element_OphNuIntraoperative_Aditionals_Aditionals_Assignment::model()->findAll('element_id = :elementId', array(':elementId' => $this->id)) as $item) {
				$existing_ids[] = $item->ophnuintraoperative_aditionals_aditionals_id;
			}
			...
			foreach ($_POST['MultiSelect_aditionals'] as $id) {
				if (!in_array($id,$existing_ids)) {
					...
					$item->ophnuintraoperative_aditionals_aditionals_id = $id;
			...
			foreach ($existing_ids as $id) {
				if (!in_array($id,$_POST['MultiSelect_aditionals'])) {
					$item = Element_OphNuIntraoperative_Aditionals_Aditionals_Assignment::model()->find('element_id = :elementId and ophnuintraoperative_aditionals_aditionals_id = :lookupfieldId',array(':elementId' => $this->id, ':lookupfieldId' => $id));
					if (!$item->delete()) {
